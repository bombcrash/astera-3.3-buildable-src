...
public class PhaseModule extends Module {
    public ModeSetting mode = new ModeSetting("Mode", "The method that will be used for phasing.", "Pearl", new String[]{"Pearl", "TP"});
    ...
    public BooleanSetting fireCharge = new BooleanSetting("Fireball", "Uses fire to bypass pearl distance checks.", false);
    public BooleanSetting alternative = new BooleanSetting("Flint", "Uses a flint and steel rather than a fire charge to perform the phase bypass.", false);
    public ModeSetting fireSwitch = new ModeSetting("FireSwap", "The mode that will be used for switching to a fire charge.", new BooleanSetting.Visibility(fireCharge, true), "Silent", new String[]{"Normal", "Silent", "AltPickup", "AltSwap"});
    ...
    public void onEnable() {
        ...
        if (mode.getValue().equalsIgnoreCase("Pearl")) {
            ...
            if (fireCharge.getValue() && mc.world.getBlockState(mc.player.getBlockPos()).isAir() && !(mc.world.getBlockState(downPosition).isReplaceable())) {
                ...
                if (chargeSlot != -1) {
                    Lyrica.ROTATION_MANAGER.packetRotate(yaw, 90);
            ...
            Lyrica.ROTATION_MANAGER.packetRotate(yaw, pitch.getValue().intValue());
