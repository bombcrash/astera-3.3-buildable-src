...
import me.lyrica.mixins.accessors.EntityVelocityUpdateS2CPacketAccessor;
...
public class VelocityModule extends Module {
    ...
    public void onTick(TickEvent event) {
        ...
        if (mode.getValue().equalsIgnoreCase("Grim") && (!pause.getValue() || Lyrica.SERVER_MANAGER.getSetbackTimer().hasTimeElapsed(100L))) {
            mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), Lyrica.ROTATION_MANAGER.getServerYaw(), Lyrica.ROTATION_MANAGER.getServerPitch(), mc.player.isOnGround(), mc.player.horizontalCollision));
    ...
    public void onPacketReceive(PacketReceiveEvent event) {
        ...
        if (event.getPacket() instanceof EntityVelocityUpdateS2CPacket packet) {
            ...
            switch (mode.getValue()) {
                ...
                case "Cancel" -> {
                    if (pause.getValue() && !Lyrica.SERVER_MANAGER.getSetbackTimer().hasTimeElapsed(100L)) return;
